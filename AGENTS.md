...
- **Lid**: the switch is `Apple SMC power/lid events`, not `Lid Switch`; there is no ACPI `/proc/acpi/button/lid` here. Binds live in `monitors.lua` and run `asahi-clamshell`, `locked = true`. logind reports `Docked=true` whenever an external is attached, so `HandleLidSwitchDocked=ignore` keeps it from suspending.
...
- **Clamshell**: lid binds run `asahi-clamshell close|open`. Close with an enabled external → eDP-1 `disabled = true` + flag `$XDG_RUNTIME_DIR/asahi-clamshell`; without one → dpms-blank only (never zero outputs). Open → eDP-1 back (mode/position from monitors.lua, scale via `saved_scale`). `apply` re-asserts on `config.reloaded` and on `monitor.removed`.
