...
- **Lid**: Apple Silicon names the switch `Apple SMC power/lid events`, not `Lid Switch`. Binds in `asahi/hypr/conf.d/monitors.lua` call `asahi-monitor-internal clamshell` (`locked = true`). There is no ACPI `/proc/acpi/button/lid` on this hardware.
...
- **Clamshell**: lid closed + external disables eDP-1 so workspaces leave the black panel; undocked lid close is DPMS + `asahi-idle-brightness` only (no lock). `SUPER+CTRL+Delete` toggles the laptop panel when docked.
