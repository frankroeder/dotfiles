...
alias klz='kgpn | fzf --preview "kubectl logs {}" --bind "f1:execute(kubectl logs {} | less -F)" --no-height --header "Press F1 for output in less"'
...
kpf(){
  ...
  kubectl port-forward $(kgpn | fzf --exit-0 --query="$2" --select-1) $1;
}
