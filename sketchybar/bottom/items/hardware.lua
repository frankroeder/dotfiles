...
cpu:subscribe('routine', function(env)
  sbar.exec('/opt/homebrew/bin/macmon pipe -s 1', function(output)
    ...
    local ecpu = math.floor(output.ecpu_usage[2] * 100)
    local pcpu = math.floor(output.pcpu_usage[2] * 100)
    ...
    if ecpu and pcpu and cpu_temp and ram_total and ram_used and swap_total and swap_used and gpu_used and gpu_temp then

      -- Update CPU
      local cpu_total = (ecpu + pcpu) / 2
      local color = colors.blue
      if cpu_total > 30 then
        if cpu_total < 60 then
          color = colors.yellow
        elseif cpu_total < 80 then
          color = colors.orange
        else
          color = colors.red
        end
      ...
      cpu:set({
        graph = { color = color },
        label = "eCPU " .. ecpu .. "% pCPU " .. pcpu .. "% " .. math.floor(cpu_temp) .. "°C",
      })
      cpu:push({ cpu_total / 100. })
