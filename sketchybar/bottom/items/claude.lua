...
local function update_quota()
  sbar.exec("$HOME/bin/claude-quota --json | jq -r '[.session.percentRemaining, .weekly.percentRemaining, .weeklySonnet.percentRemaining, .session.resetText] | join(\"|\")'", function(result)
