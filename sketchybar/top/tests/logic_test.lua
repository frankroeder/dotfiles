...
ok(right["widgets.vpn"], "right vpn")
...
ok(not read_src("sketchybar/top/items/calendar.lua"):find("%a %d %b  -  %H:%M", 1, true), "calendar has no dash between date and time")
ok(not read_src("sketchybar/top/items/calendar.lua"):find "widgets.calendar.date", "calendar is not stacked")
...
if f then
  ...
  ok(init_src:find 'require "items.vpn"', "init requires vpn")
...
ok(not read_src("sketchybar/bottom/items/init.lua"):find "items.vpn", "bottom init does not require vpn")
ok(read_src("sketchybar/top/items/vpn.lua"):find "widgets.vpn", "vpn widget lives on the top bar")
