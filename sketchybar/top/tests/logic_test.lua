...
local ok, eq, fail = T.ok, T.eq, T.fail
local root = T.root
...
ok(read_src("sketchybar/top/items/volume.lua"):find "hover_label", "volume % on hover")
ok(read_src("sketchybar/top/items/volume.lua"):find "drawing = false", "volume label hidden idle")
...
ok(read_src("sketchybar/top/items/mic.lua"):find "hover_label", "mic % on hover")
ok(read_src("sketchybar/top/items/mic.lua"):find "drawing = false", "mic label hidden idle")
...
ok(read_src("sketchybar/top/items/wifi.lua"):find "set_hot", "wifi hover reveals rates")
ok(read_src("sketchybar/top/items/network.lua"):find "drawing = false", "network rates hidden idle")
ok(read_src("sketchybar/top/items/network.lua"):find "if hot then", "network paints only while hovered")
...
eq(island_style.bar().border_color, 0xff000000, "island border is notch-black")
eq(island_style.bar().border_width, 0, "island has no themed ring")
...
ok(island_style.bar().border_color ~= settings.theme.border, "island border is not the themed ring")
