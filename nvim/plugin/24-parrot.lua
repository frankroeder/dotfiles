...
if not require("pack_helpers").add_runtime("~/Documents/luapos/parrot.nvim") then
  return
end
...
local function add_ollama_provider(opts)
    ...
    opts.providers["ollama"] = {
      ...
      get_available_models = function(self)
        ...
        if parsed_response == "" then
          logger.debug("Ollama server not running on " .. url)
          return {}
        end
